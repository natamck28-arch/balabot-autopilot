...
function convoSystem(brand) {
  ...
  const connectionNote = connected
    ? `מצב החיבור (עובדה, אל תסתור אותה): החשבון של העסק כבר **מחובר לאינסטגרם @${brand.igUsername}** ואתה מוכן לפרסם עבורו. לעולם אל תגיד שאתה "לא מחובר" או "לא מחובר בפועל", אל תבקש שם משתמש של אינסטגרם, ואל תציע לחבר חשבון — זה כבר בוצע. אם הלקוח רוצה לפרסם: פשוט בקש ממנו לשלוח תמונה (ואם רוצה סטורי — שיכתוב "סטורי"). אתה גם לא שולח קישורים.`
    : `מצב החיבור: החשבון עדיין לא חובר לאינסטגרם. אם הלקוח רוצה לחבר — הנחה אותו לסיים את קישור ההרשמה הקצר שקיבל מאיש המכירות. אתה עצמך לא מחבר חשבונות דרך הצ'אט ולא שולח קישורים.`;
